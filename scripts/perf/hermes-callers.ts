...
function callersOf(target: string, depth = 4) {
  ...
  for (const [id, ms] of self) {
    ...
    let found = false;
    while (cur != null) {
      if (fn(cur).includes(target)) { found = true; break; }
      cur = parent.get(cur);
    }
    if (!found) continue;
    const chain: string[] = [];
    let up: number | undefined = parent.get(cur as number);
    for (let d = 0; d < depth && up != null; d++) { chain.push(fn(up)); up = parent.get(up); }
...
const OURS = /^(ChapterReader|ChapterPage|SimpleChapterPager|ParagraphText|VMText|HighlightedText|useParagraphLayout|compileParagraph|BibleChapterScreen|Markdown)/;
...
for (const [id, ms] of self) {
  ...
  while (cur != null) {
    const name = fn(cur);
    if (OURS.test(name) && !seen.has(name)) { seen.add(name); totals.set(name, (totals.get(name) ?? 0) + ms); }
    cur = parent.get(cur);
  }
}
console.log(`\n=== our components, total subtree time (of ${total.toFixed(0)}ms) ===`);
for (const [k, ms] of [...totals].sort((a, b) => b[1] - a[1]).slice(0, 14)) {
  console.log(`  ${ms.toFixed(0).padStart(5)}ms  ${((ms / total) * 100).toFixed(1).padStart(4)}%  ${k}`);
}
