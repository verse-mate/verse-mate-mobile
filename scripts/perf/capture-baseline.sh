...
adb_sh() { pcrun "& '$PC_ADB' -s \$env:VM_DEVICE $*"; }

# --- 1. device ---------------------------------------------------------------

step 'Discovering the phone over mDNS'
# The wireless-debug port rotates every time the phone toggles the setting, so
# never hard-code it — rediscover, then connect. See versemate-phone-devloop.
DEVICE="$(pcrun "
  \$adb = '$PC_ADB'
  \$found = \$null
  & \$adb mdns services 2>\$null | Select-String '_adb-tls-connect' | ForEach-Object {
    if (\$_ -match '(\d+\.\d+\.\d+\.\d+:\d+)') {
      \$r = & \$adb connect \$matches[1] 2>&1
      if (\$r -notmatch 'refused|failed') { \$found = \$matches[1] }
    }
  }
  if (-not \$found) {
    # Already-connected devices won't necessarily re-broadcast.
    \$found = (& \$adb devices | Select-String ':\d+\s+device' | ForEach-Object {
      (\$_ -split '\s+')[0] }) | Select-Object -First 1
  }
  if (\$found) { \"DEVICE=\$found\" }
" 2>&1 | grep -oP 'DEVICE=\K\S+' | head -1)" || true

[[ -n "${DEVICE:-}" ]] || die "No device found.

Ask the operator to turn on Wireless Debugging (Developer options), then re-run.
Pairing is sticky across off/on cycles, so no pairing code should be needed.
If every discovered address refuses the connection, pairing was wiped and a
fresh 'adb pair <ip:port> <code>' is required."

step "Device: $DEVICE"
pcrun "\$env:VM_DEVICE = '$DEVICE'" >/dev/null
...
DEBUGGABLE="$(adb_sh "shell 'pm dump $PKG | grep -m1 flags'" 2>&1 | grep -c DEBUGGABLE || true)"
...
if ! pcrun "cd '$PC_REPO'; maestro --device $DEVICE test '$FLOW'" 2>&1 | tee "$OUT/maestro.log"; then
