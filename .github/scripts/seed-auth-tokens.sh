...
# Write access token + cached user via temp SQL file (avoids shell quoting issues).
# Refresh tokens eliminated per D-005 — access token IS the persistent session token.
...
# Verify
KEY_COUNT=$(adb shell "sqlite3 ${DB_PATH} \"SELECT COUNT(*) FROM catalystLocalStorage WHERE key IN ('versemate_access_token', 'versemate_cached_user', 'versemate_e2e_mode');\"" | tr -d '\r')
